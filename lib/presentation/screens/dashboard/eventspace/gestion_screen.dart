...
class EventSpacesScreen extends StatefulWidget {
  const EventSpacesScreen({super.key});
...
class _EventSpacesScreenState extends State<EventSpacesScreen> {
  ...
  void _onScroll() {
    if (_scrollController.offset > EventSpacesStyles.scrollThreshold &&
        !_isScrolled) {
      setState(() => _isScrolled = true);
    } else if (_scrollController.offset <= EventSpacesStyles.scrollThreshold &&
        _isScrolled) {
  ...
  Widget _buildCircularButton({
    ...
    return Container(
      width: EventSpacesStyles.circularButtonSize,
      height: EventSpacesStyles.circularButtonSize,
      margin: EdgeInsets.symmetric(
          horizontal: EventSpacesStyles.circularButtonMargin),
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context) {
    return PreferredSize(
      preferredSize: Size.fromHeight(EventSpacesStyles.appBarTotalHeight),
      child: AnimatedContainer(
        ...
        child: AppBar(
          ...
          elevation: 0,
          toolbarHeight: EventSpacesStyles.appBarTotalHeight,
          automaticallyImplyLeading: false,
          flexibleSpace: Column(
            children: [
              Container(
                width: double.infinity,
                height: EventSpacesStyles.bannerHeight,
              ),
              SafeArea(
                child: Column(
                  children: [
                    SizedBox(
                      height: EventSpacesStyles.buttonRowHeight,
                      child: Padding(
                        padding: EdgeInsets.symmetric(
                            horizontal: EventSpacesStyles.horizontalPadding),
                        child: Row(
                          children: [
                    ...
                    SizedBox(
                        height: EventSpacesStyles.spaceBetweenButtonAndTitle),
                    Container(
                      width: double.infinity,
                      height: EventSpacesStyles.titleContainerHeight,
                      margin: EdgeInsets.symmetric(
                          horizontal: EventSpacesStyles.horizontalPadding),
                      padding: EventSpacesStyles.titlePadding,
                      decoration: BoxDecoration(
                        color: Colors.white,
                        borderRadius: BorderRadius.circular(
                            EventSpacesStyles.borderRadius),
                        border: Border.all(color: Colors.grey[300]!),
                      ),
                      child: const Text(
      ...
      body: SingleChildScrollView(
        ...
        child: Padding(
          padding: EdgeInsets.only(
            top: EventSpacesStyles.appBarTotalHeight + 20,
            left: 20,
            right: 20,
            bottom: 20,
          ),
